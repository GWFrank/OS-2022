...
void schedule(void) {
    ...
    if (is_thread_start == 0 || current_thread->is_yield || current_thread->is_exited) {
        struct thread *min_thrd;
        if (is_thread_start == 1 && current_thread->is_exited)
            min_thrd = current_thread->next; // skip current thread bc it's exiting
        else
            min_thrd = current_thread;
        struct thread *thrd_ptr = min_thrd->next;
        while (thrd_ptr != current_thread) {
            if (
                thrd_ptr->remain_execution_time < min_thrd->remain_execution_time
                || ( thrd_ptr->remain_execution_time == min_thrd->remain_execution_time && thrd_ptr->ID < min_thrd->ID)
            ) {
                min_thrd = thrd_ptr;
            }
